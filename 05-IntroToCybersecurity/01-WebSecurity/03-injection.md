...
- The shell tries to run the `/user/bin/whoami` program
...
- When we execute `foo("AAAAAAAAAAAAAAAABBBBBBBB!", 25);` with 16 A's followed by 8 B's followed by an exclamation point, we leave the buffer context and go into the stack context, and in this stack context the function call overwrites 1 byte of the end of the original return address from `42` to `21`
